...
add_task(async function test_records_profile_with_different_install() {
  ...
  let otherInstallDir = Cc["@mozilla.org/file/local;1"].createInstance(
    Ci.nsIFile
  );
  otherInstallDir.initWithPath("/nonexistent/install/path");
  ...
  await GleanPings.newProfile.testSubmission(
    () => {
      ...
      Assert.greaterOrEqual(found.last_used_seconds, 0);
